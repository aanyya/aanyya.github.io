...
// Получаем все товары для отображения (только очки для зрения)
try {
    $allProducts = []; // Все товары без фильтрации (для поиска)
    $products = [];    // Товары после применения фильтров
    $table = 'items_vision';

    // Получаем все возможные фильтры для очков для зрения
    ...
    $sql = "SELECT id, name, CAST(price AS DECIMAL(10,2)) as price, image, quantity, structure,
            'vision' AS category, type, color, shape, rating, popularity FROM $table";
    ...
    $popularSearches = [
        ["text" => "Очки для зрения", "category" => "vision"],
        ["text" => "Корригирующие очки", "category" => "vision"],
        ["text" => "Очки с диоптриями", "category" => "vision"],
        ["text" => "Оправы для очков", "category" => "vision"],
        ["text" => "Seen очки", "category" => "vision"]
    ];
...
<?php include 'header.php'; ?>
    ...
    <div class="block-collection">
        <div class="breadcrumbs">
            ...
            <img src="img/arrow-right.svg" alt="">
            <a class="hierarchy" href="sun.php">Очки для зрения</a>
        </div>
        <h1 class="gradient">Очки для зрения</h1>
