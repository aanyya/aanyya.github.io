...
if (isset($_GET['toggle_favorite'])) {
    ...
    $productCategory = 'lenses'; // Фиксированная категория для этой страницы

    try {
        // Проверяем существует ли товар
        $stmt = $pdo->prepare("SELECT id FROM items_lenses WHERE id = ?");
...
try {
    ...
    // Получаем все возможные фильтры для линз
    $allTypes = $pdo->query("SELECT DISTINCT type FROM items_lenses WHERE type IS NOT NULL AND type != ''")->fetchAll(PDO::FETCH_COLUMN);
    $allColors = $pdo->query("SELECT DISTINCT color FROM items_lenses WHERE color IS NOT NULL AND color != ''")->fetchAll(PDO::FETCH_COLUMN);

    // Получаем товары из таблицы линз
    $sql = "SELECT id, name, CAST(price AS DECIMAL(10,2)) as price, image, quantity, structure,
            'lenses' AS category, type, color, rating, popularity FROM items_lenses";
    ...
    $popularSearches = [
        ...
        ["text" => "Линзы на месяц", "category" => "lenses"],
        ["text" => "Цветные линзы", "category" => "lenses"],
        ["text" => "Линзы для чувствительных глаз", "category" => "lenses"]
    ];
