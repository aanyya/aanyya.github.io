...
if (isset($_GET['toggle_favorite'])) {
    ...
    $productCategory = 'sun'; // Фиксируем категорию как солнечные очки

    try {
        // Проверяем существует ли товар
        $stmt = $pdo->prepare("SELECT id FROM items_sun WHERE id = ?");
...
// Получаем все товары для отображения (только солнечные очки)
try {
    $allProducts = []; // Все товары без фильтрации (для поиска)
    $products = [];    // Товары после применения фильтров
    $table = 'items_sun';

    // Получаем все возможные фильтры для солнечных очков
    ...
    try {
        $sql = "SELECT id, name, CAST(price AS DECIMAL(10,2)) as price, image, quantity, structure,
                'sun' AS category, type, color, shape, rating, popularity FROM $table";
    ...
    $popularSearches = [
        ["text" => "Классические солнцезащитные", "category" => "sun"],
        ["text" => "Авиаторы", "category" => "sun"],
        ["text" => "Wayfarer", "category" => "sun"],
...
<?php include 'header.php'; ?>
    ...
    <div class="block-collection">
        <div class="breadcrumbs">
            ...
            <img src="img/arrow-right.svg" alt="">
            <a class="hierarchy" href="sun.php">Солнцезащитные очки</a>
        </div>
        <h1 class="gradient">Солнцезащитные очки</h1>
