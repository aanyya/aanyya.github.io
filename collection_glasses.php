...
if (isset($_GET['toggle_favorite'])) {
    ...
    if (!array_key_exists($productCategory, $tableMap)) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT id FROM {$tableMap[$productCategory]} WHERE id = ?");
...
if (isset($_SESSION['user_id'])) {
    ...
    $stmt = $pdo->prepare("SELECT product_id, product_category FROM favorites WHERE user_id = ?");
    $stmt->execute([$userId]);
...
try {
    ...
    $tables = [
     ...
     // Получаем все возможные фильтры (убираем дублирование)
    $allTypes = [];
    $allShapes = [];
    $allColors = [];
    ...
    foreach ($tables as $category => $table) {
        // Получаем типы
        $types = $pdo->query("SELECT DISTINCT type FROM $table WHERE type IS NOT NULL AND type != ''")->fetchAll(PDO::FETCH_COLUMN);
        $allTypes = array_unique(array_merge($allTypes, $types));

        // Получаем формы (если есть в таблице)
        if ($category != 'lenses') {
            $shapes = $pdo->query("SELECT DISTINCT shape FROM $table WHERE shape IS NOT NULL AND shape != ''")->fetchAll(PDO::FETCH_COLUMN);
            $allShapes = array_unique(array_merge($allShapes, $shapes));
        }

        // Получаем цвета
        $colors = $pdo->query("SELECT DISTINCT color FROM $table WHERE color IS NOT NULL AND color != ''")->fetchAll(PDO::FETCH_COLUMN);
        $allColors = array_unique(array_merge($allColors, $colors));
        // Получаем товары
        try {
            $sql = "SELECT id, name, CAST(price AS DECIMAL(10,2)) as price, image, quantity, structure,
                    IFNULL(category, :category) AS category, type, color, shape, rating, popularity FROM $table";
            ...
            foreach ($categoryProducts as $product) {
                ...
                if (!isset($product['shape'])) $product['shape'] = '';
                if (!isset($product['rating'])) $product['rating'] = 4.5;
                if (!isset($product['popularity'])) $product['popularity'] = 0;

                $allProducts[] = $product; // Добавляем в массив всех товаров

                // Применяем поисковый запрос сразу при загрузке товаров
                if (empty($searchQuery)) {
                    $products[] = $product;
                } elseif (stripos($product['name'], $searchQuery) !== false ||
            ...
            error_log("Ошибка при загрузке товаров из $table: " . $e->getMessage());
            continue;
        }
    }
    ...
    $popularSearches = [
        ["text" => "Солнцезащитные очки", "category" => "sun"],
        ["text" => "Оправы для зрения", "category" => "vision"],
        ["text" => "Контактные линзы", "category" => "lenses"],
        ["text" => "iWear Activ", "category" => "lenses"],
        ["text" => "Seen очки", "category" => "sun"]
    ];
...
<div class="block-collection">
    <div class="breadcrumbs">
        <a class="hierarchy" href="">Главная</a>
        <img src="img/arrow-right.svg" alt="">
        <a class="hierarchy" href="">Каталог</a>
    </div>
    <h1 class="gradient">Коллекция очков в Москве</h1>
...
<div class="filter-group">
    ...
    <div class="filter-options">
        <?php foreach ($allTypes as $type):
            $isChecked = isset($_GET['types']) && in_array($type, (array)$_GET['types']);
        ?>
...
<div class="filter-group">
    ...
    <div class="filter-options">
        <?php foreach ($allShapes as $shape):
            $isChecked = isset($_GET['shapes']) && in_array($shape, (array)$_GET['shapes']);
        ?>
            <label class="filter-option">
...
<div class="filter-group">
    ...
    <div class="filter-options">
        <?php foreach ($allColors as $color):
            $isChecked = isset($_GET['colors']) && in_array($color, (array)$_GET['colors']);
        ?>
            <label class="filter-option">
...
?>
    <div class="product-card <?= $isOutOfStock ? 'out-of-stock' : '' ?>"
        ...
        <a href="?toggle_favorite=1&item_id=<?= $product['id'] ?>&item_type=<?= $product['category'] ?>"
...
<script>
    ...
    document.querySelectorAll('.filter-option input[type="checkbox"]').forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            ...
            // Добавляем только активные чекбоксы
            form.querySelectorAll('input[type="checkbox"]:checked').forEach(checked => {
                searchParams.append(checked.name, checked.value);
            });

            // Добавляем сортировку
            const sortBy = form.querySelector('#sort-by').value;
            const sortOrder = form.querySelector('#sort-order').value;
            if (sortBy !== 'default') {
    ...
    function showSuggestions(results) {
        ...
        for (const [category, products] of Object.entries(groupedResults)) {
            ...
            products.slice(0, 3).forEach(product => {
                ...
                item.href = `product_detail.php?id=${product.id}&category=${product.category}`;
