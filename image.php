...
if (isset($_GET['toggle_favorite'])) {
    ...
    $productCategory = 'image'; // Только имиджевые очки

    try {
        // Проверяем существует ли товар
        $stmt = $pdo->prepare("SELECT id FROM items_image WHERE id = ?");
        ...
        error_log("Favorite error: User $userId, Product $productId. Error: " . $e->getMessage());
        header("Location: " . $_SERVER['HTTP_REFERER']);
        exit();
    }
}
...
if (isset($_SESSION['user_id'])) {
    ...
    $stmt = $pdo->prepare("SELECT product_id, product_category FROM favorites WHERE user_id = ? AND product_category = 'image'");
    $stmt->execute([$userId]);
    ...
    $sql = "SELECT oi.item_id as product_id, oi.product_category
            FROM order_items oi
            JOIN orders o ON oi.order_id = o.id
            WHERE o.user_id = ? AND oi.product_category = 'image' AND o.status = 'completed'";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$userId]);
...
try {
    ...
    // Получаем только имиджевые очки
    $table = 'items_image';

    // Получаем все возможные фильтры
    $allTypes = $pdo->query("SELECT DISTINCT type FROM $table WHERE type IS NOT NULL AND type != ''")->fetchAll(PDO::FETCH_COLUMN);
    $allShapes = $pdo->query("SELECT DISTINCT shape FROM $table WHERE shape IS NOT NULL AND shape != ''")->fetchAll(PDO::FETCH_COLUMN);
    $allColors = $pdo->query("SELECT DISTINCT color FROM $table WHERE color IS NOT NULL AND color != ''")->fetchAll(PDO::FETCH_COLUMN);

    // Получаем товары
    $sql = "SELECT id, name, CAST(price AS DECIMAL(10,2)) as price, image, quantity, structure,
            'image' AS category, type, color, shape, rating, popularity FROM $table";
    ...
    $popularSearches = [
        ["text" => "Классические оправы", "category" => "image"],
        ["text" => "Модные очки", "category" => "image"],
        ["text" => "Очки для стиля", "category" => "image"],
        ["text" => "Дизайнерские очки", "category" => "image"],
        ["text" => "Очки для мужчин", "category" => "image"]
    ];
...
<head>
    ...
    <meta name="description" content="Каталог стильных имиджевых очков и оправ по выгодной цене в Москве.
    ✅ Большой ассортимент - доступные цены за качество!
    ✅ Заказать и подобрать очки - вы можете у нас на сайте.">
    <title>luw – имиджевые очки</title>
...
<?php include 'header.php'; ?>
    ...
    <div class="block-collection">
        <div class="breadcrumbs">
            ...
            <img src="img/arrow-right.svg" alt="">
            <a class="hierarchy" href="image.php">Имиджевые очки</a>
        </div>
        <h1 class="gradient">Имиджевые очки</h1>
